...
from exception_layer.generic_exception.generic_exception import GenericException as  DataTransformationException
...
class DataTransform:
    ...
    def __init__(self, project_id, executed_by, execution_id, cloud_storage, socket_io=None):
        try:
            ...
            self.logger = AppLogger(project_id=project_id, executed_by=executed_by, execution_id=execution_id,
                                    socket_io=socket_io)
            self.goodDataPath = self.initializer.get_training_good_raw_data_file_path(project_id)
            self.project_id = project_id
            self.logger.log_database = self.initializer.get_training_database_name()
            self.file_manager = FileManager(cloud_provider=cloud_storage)
        except Exception as e:
            data_transformation_exception = DataTransformationException(
                "Failed during instantiation of object in module [{0}] class [{1}] method [{2}]"
                    .format(self.__module__, DataTransform.__name__,
                            "__init__"))
    ...
    def replace_missing_with_null_fraud_detection(self):
        try:
            self.logger.log_collection_name=self.initializer.get_data_transform_log_collection_name()
            self.logger.log("Started replacing missing value with null.")
            response = self.file_manager.list_files(self.goodDataPath)
            if not response['status']:
                return True
            ...
            data_transformation_exception = DataTransformationException(
                "Failed during data transformation of object in module [{0}] class [{1}] method [{2}]"
                    .format(self.__module__, DataTransform.__name__,
                            self.replace_missing_with_null_fraud_detection.__name__))
            raise Exception(data_transformation_exception.error_message_detail(str(e), sys)) from e
