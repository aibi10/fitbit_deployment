...
class TrainingModel:
    ...
    def training_model(self):
        ...
        try:
            ...
            data=preprocessor.encode_categorical_value_forest_cover(data)
            ...
            for i in list_of_clusters:
                ...
                x_train=preprocessor.scale_data_forest_cover(x_train)
                x_test=preprocessor.scale_data_forest_cover(x_test)
                # getting the best model for each of the clusters

                model_finder = tuner.ModelFinder(project_id=self.project_id, file_object=self.file_manager,
                                                 logger_object=self.log_writer)  # object initialization

                best_model_name, best_model = model_finder.get_best_model_forest_cover(x_train, y_train, x_test, y_test,cluster_no=str(i))
