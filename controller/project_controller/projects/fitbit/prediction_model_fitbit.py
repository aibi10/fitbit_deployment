...
class Prediction:
    ...
    def prediction_from_model(self):
        ...
        try:
            ...
            data = preprocessor.drop_unnecessary_columns(data, ['Id', 'ActivityDate', 'TotalDistance', 'TrackerDistance'])
            data=preprocessor.replace_invalid_values_with_null_fitbit(data)
            is_null_present,cols_with_missing_values = preprocessor.is_null_present_in_columns(data)
            if is_null_present:
                data=preprocessor.impute_missing_values(data)


            data_scaled = pandas.DataFrame(preprocessor.standard_scaling_data_of_column(data), columns=data.columns)
