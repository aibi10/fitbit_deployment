    ...
    def prediction_from_model(self):
        ...
        try:
            ...
            data = preprocessor.remove_columns(data, ["Index_Product", "sku",
                                                      "oe_constraint"])  # removing oe_constraint as it was removed in training
            data=preprocessor.encode_categorical_values_back_order_prediction(data)
            ...
            data=preprocessor.scale_numerical_columns_back_order(data)
            data=preprocessor.pca_transform_back_order(data)
            file_loader = file_methods.FileOperation(project_id=self.project_id, file_object=self.file_object,
                                                     logger_object=self.log_writer)


            model_name = file_loader.find_correct_model_without_cluster()
