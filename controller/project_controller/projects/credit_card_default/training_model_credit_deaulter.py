...
class TrainingModel:
    ...
    def training_model(self):
        ...
        try:
            ...
            is_null_present, cols_with_missing_values = preprocessor.is_null_present_in_columns(X)


            if is_null_present:
                X = preprocessor.impute_missing_values_mushroom(X, cols_with_missing_values)
            ...
            for i in list_of_clusters:
                ...
                x_train=preprocessor.scale_numerical_columns_credit_default_or_income_prediction(x_train)
                x_test=preprocessor.scale_numerical_columns_credit_default_or_income_prediction(x_test)
                model_finder = tuner.ModelFinder(project_id=self.project_id, file_object=self.file_manager,
                                                 logger_object=self.log_writer)  # object initialization


                best_model_name, best_model = model_finder.get_best_model_credit_deaulter(x_train, y_train, x_test, y_test,
                                                                                          cluster_no=str(i))
